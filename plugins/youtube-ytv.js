...
let handler = async (m, { conn, args, usedPrefix, command }) => {
	...
	await m.reply(wait);
	try {
		let anu = await( await ytdl(`https://api.betabotz.eu.org/api/download/ytmp4?url=${args[0]}&apikey=${lann}`)).json();
		let txt = `*${anu.title}*\n\n`
		+ `⌲ Resolution : 360p\n`
		+ `⌲ Duration : ${anu.duration}`
		await conn.sendFile(m.chat, anu.mp4, `${anu.title}.mp4`, txt, m)
    } catch (e) {
		console.log(e)
	    throw global.eror
    }
